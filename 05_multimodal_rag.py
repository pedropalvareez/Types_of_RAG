...
# ---------------------------------------------------------------------------
# Sample multimodal corpus (simulated as structured objects)
# ---------------------------------------------------------------------------
@dataclass
class Document:
    doc_id: str
    modality: Modality
    content: Any          # str for text, dict for image/table metadata
    description: str      # textual description used for display
...
# Each modality uses a different "projection" so they share a common space.
# In a real system this would be CLIP / ColPali / ImageBind.
# ---------------------------------------------------------------------------
class MultimodalEmbedder:
    """
    Mock embedder that maps any modality to a shared 64-dim embedding space.
    For images/tables, embeds the textual description/caption.
    For text, embeds the content directly.
    Modality-specific offset simulates cross-modal alignment.
    """
    DIM = 64
    MODALITY_OFFSET = {
        Modality.TEXT:  np.zeros(64),
        Modality.IMAGE: np.ones(64) * 0.05,   # small shared-space shift
...
class MultimodalVectorIndex:
    ...
    def search(self, query: str, top_k: int = 5,
               modality_filter: Modality | None = None) -> list[tuple[Document, float]]:
...
# ---------------------------------------------------------------------------
# Mock Multimodal LLM (vision + text)
# ---------------------------------------------------------------------------
...
def run_multimodal_rag(query: str, top_k: int = 5):
    ...
    # Step 1: Build unified multimodal index
    ...
    # Step 2: Query understanding — detect modality preferences
    q_lower = query.lower()
    ...
    # Step 3: Retrieve across all modalities
    ...
    # Step 4: Optionally boost preferred modalities
    ...
    # Step 5: Generate multimodal answer
