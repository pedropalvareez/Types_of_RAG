...
# ---------------------------------------------------------------------------
# Fake embedder (same pattern as hybrid RAG)
# ---------------------------------------------------------------------------
class FakeEmbedder:
    DIM = 32
...
class ToolResult:
    ...
    query_used: str
    results: Any
    confidence: float  # 0–1 estimate of how useful this result was
...
class WebSearchTool:
    ...
    def run(self, query: str) -> ToolResult:
        # Find best matching web result via keyword overlap
...
def planner_agent(query: str) -> list[PlanStep]:
    """Rule-based planner that maps query keywords to tool selections."""
    plan = []
    q = query.lower()
    # Always try vector search for conceptual questions
    plan.append(PlanStep("vector_search", query))
    # If the question mentions numbers, dates, comparisons → SQL
    if any(w in q for w in ["2024", "2023", "released", "params", "size"]):
        plan.append(PlanStep("sql_database", query))
    # If mentions benchmarks, comparisons, latest → web search
    if any(w in q for w in ["benchmark", "compare", "latest", "recent", "best"]):
        plan.append(PlanStep("web_search", "benchmark comparison LLMs"))
    return plan

# ---------------------------------------------------------------------------
# Reasoner Agent: synthesises tool results into a final answer
# ---------------------------------------------------------------------------
