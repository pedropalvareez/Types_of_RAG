...
# Simulated web search results (fallback)
WEB_RESULTS = {
...
def evaluate_relevance(query: str, docs: list[dict]) -> tuple[Verdict, float, str]:
    """
    Mock grader: checks keyword overlap between query and retrieved docs.
...
def rewrite_query(original_query: str) -> str:
    """Expands/clarifies the query by adding synonyms or context keywords."""
    rewrites = {
    ...
    for key, rewrite in rewrites.items():
        if key in q_lower:
            return rewrite
    # Generic rewrite: append "details overview"
...
def run_corrective_rag(query: str):
    ...
    # Step 3: Branch based on verdict
...
# ---------------------------------------------------------------------------
# Run three scenarios to demonstrate all branches
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    # Scenario A: should hit CORRECT or AMBIGUOUS (query about anthropic)
    run_corrective_rag("Who founded Anthropic and what do they build?")
    print()
    # Scenario B: vague query → likely AMBIGUOUS
    run_corrective_rag("tell me about claude")
    print()
    # Scenario C: off-topic query → INCORRECT → web fallback
